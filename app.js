...
// Geeft lege string als qty 0 is — voor gebruik in cash-cellen
function qtyOrBlank(value){
  const rounded = round2(Number(value) || 0);
  if (rounded === 0) return "";
  return Number.isInteger(rounded) ? String(rounded) : String(rounded);
}
...
// Zorgt enkel dat invoiceDate bestaat — geen factuurnummer toewijzing.
// Gebruik deze overal BEHALVE in calculateSettlement.
function ensureInvoiceDate(settlement){
  if (!settlement) return;
  if (!settlement.invoiceDate){
...
// Volledig: kent ook factuurnummer toe. Alleen aanroepen vanuit calculateSettlement.
function ensureSettlementInvoiceDefaults(settlement, settlements = state.settlements || []){
  if (!settlement) return;
  ensureInvoiceDate(settlement);
...
function settlementPaymentState(settlement){
  const { invoiceTotal, cashTotal, invoiceSubtotal, invoiceVat } = getSettlementTotals(settlement);
  ...
  return { invoiceTotal, cashTotal, invoiceSubtotal, invoiceVat, hasInvoice, hasCash, isPaid };
}
...
function calculateSettlement(settlement){
  ...
  // Zorg dat allocations bestaan als ze er nog niet zijn (migratie)
  if (!settlement.allocations) buildAllocationsFromLogs(settlement);
...
function renderSettlementSheet(id){
  ...
  const renderAllocationControls = ({ key, bucket, qty })=>{
    ...
    const displayQty = bucket === 'cash' ? qtyOrBlank(qty) : esc(String(formatQuickQty(qty)));
    return `<div class="allocation-controls" data-bucket="${bucket}">
      ...
      <div class="allocation-value mono tabular">${displayQty}</div>
  ...
  if (isEdit){
    ...
    // (Herbereken knop is verwijderd — allocations worden automatisch bijgewerkt bij log-koppeling)
